...
const SubInput = (props) => {
  ...
  return (
    <div className='flex flex-col gap-y-3 relative overflow-hidden'>
        <label className='slate-500 '>{props.label}</label>
        <div className='flex'>
          <input type="number" value={props.value} onChange={handleChange} className={`w-55 h-10 p-6 rounded-lg border-1 border-black focus:outline-hidden focus:border-[#d7da2f]   ${props.errorMessage ? 'border-red-500' : 'border-black'}`}/>
