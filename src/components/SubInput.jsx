...
const SubInput = (props) => {
  ...
  return (
    <div className='flex flex-col gap-y-3 relative overflow-hidden w-full'>
        <label htmlFor={props.label} className='slate-500 '>{props.label}</label>
        <div className='flex'>
          <input id={props.label} type="number" value={props.value} onChange={handleChange} className={`w-full h-10 p-6 rounded-lg border-1 border-black focus:outline-hidden focus:border-[#d7da2f]   ${props.errorMessage ? 'border-red-500' : 'border-black'}`}/>
